...
MANIFEST = os.path.join(BASE_DIR, "videos/07_generation_logs/simswap_bulk_manifest.csv")
LOG_FILE = os.path.join(BASE_DIR, "videos/07_generation_logs/simswap_bulk.log")
...
for idx, job in df.iterrows():
    ...
    bash_script = f'export PATH="$HOME/.pyenv/shims:$HOME/.pyenv/bin:$PATH" && cd /mnt/c/Users/User/Work/deep-fake-detection/download-pipeline/yt-shorts-pipeline/SimSwap && ../simswap_venv/bin/python test_video_swapsingle.py --crop_size 224 --use_mask --name people --Arc_path ../arcface_model/arcface_checkpoint.tar --pic_a_path "{wsl_path(job["ref_path"])}" --video_path "{wsl_path(job["source_path"])}" --output_path "{wsl_path(job["output_path"])}" --temp_path ./tmp'
